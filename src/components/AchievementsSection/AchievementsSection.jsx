          ...
          >
            {achievements.map((ach) => (
              <motion.div 
                ...
                className="bg-white/5 backdrop-blur-sm rounded-2xl p-6 border border-white/10 hover:border-white/20 transition-colors duration-300 flex flex-col h-full"
              >
                {ach.imageUrl ? (
                  ...
                  <div className="w-12 h-12 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-white/70 text-2xl mb-5 shrink-0">
                    🏆
                  </div>
                )}
                ...
                <div className="flex items-start justify-between gap-4 mb-2">
                  ...
                  {ach.year && <span className="text-xs font-bold bg-white/5 text-white/60 px-2 py-1 rounded-md border border-white/10 shrink-0">{ach.year}</span>}
                </div>
                
                {ach.organization && (
                  <p className="text-white/55 text-sm font-semibold mb-3">{ach.organization}</p>
                )}
